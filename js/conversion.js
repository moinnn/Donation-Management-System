// Javascript Function for Currency Conversion
...
for(i=0;i<numlen;i++)
{
    switch(i){case 0:if((rev[i]==0) || (rev[i+1]==1)){word[j]='';
}
else{
    word[j]=once[rev[i]];
}
...
else {word[j]=once[rev[i]]+ " Hundred";} break;case 3:if(rev[i]==0 || rev[i+1]==1){word[j]='';
} 
...
document.getElementById('number').value=finalw;
}
