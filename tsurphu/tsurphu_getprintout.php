...
<body style="margin: 0px !important;">
 ...
 if (isset($_GET['receipt_no'])) {
                    ...
                    $receipt_no = stripcslashes($_GET['receipt_no']);
...
//$find = mysql_query("SELECT a.*, c.*, d.*, p.* FROM ".TSURPHU." a INNER JOIN ".CURRENCY." c using(currency_id) INNER JOIN ".DONATION." d using(donation_id) INNER JOIN ".PAYMENT." p using(payment_id) WHERE `name` LIKE '%$search%'") or die(mysql_error());
                    ...
                    $find = mysql_query("SELECT * FROM " . TSURPHU . " WHERE `receipt_no` = '$receipt_no'") or die(mysql_error());
...
<tr>
    ...
    <br>
                        ...
                        <tr style="margin-top: 60px;">
                                <td style="width: 90%;"><span class="tibetan-small">དད་འབུལ་བའི་བློ་མོས་བཞིན་དད་རྫས་རྣམས་ཆུད་མི་འཛའ་བར་ཆོས་ཕྱོགས་དང་འབྲེལ་བའི་དགེ་རྩ་ཁོ་ནའི་ཕྱིར་སྨིན་གཏོང་གང་དགེ་ཞུ་རྒྱུ་བཅས། ༧དཔལ་ཀརྨ་པའི་སྤྱི་ལས་ཁང་ནས། </span></td>
                                
                            </tr>
                         
                            <tr style="margin-top: 60px;">
                                <td style="width: 90%;"><span class="english-small">Offerings will be used for charitable & religious purposes and to sustain Monasteries & Institutes under the Tsurphu Labrang or according to the specific wishes of the donor.</span></td>
                         </tr>
<tr>
